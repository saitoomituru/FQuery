...
describe("FQueryNodePanel", () => {
  it("Q tabをqSchemaから生成し、変更をknown pathだけのfam.patchとしてrequestする", async () => {
    ...
    const setEvent = wrapper.emitted("event")![0]![0] as { property: string; value: FamDraftPatch };
    ...
    const insertEvent = wrapper.emitted("event")![1]![0] as { value: FamDraftPatch };
    expect(insertEvent.value.operations).toEqual([{ op: "insert", path: "/Q/top_k", value: 5 }]);
    const applied = previewFamDraftPatch(openFamText(JSON.stringify(fam)), insertEvent.value, { validate: validateFamJson });
  ...
  it("plugin A→B切替で認識fieldが入れ替わってもcanonical FAMは同一のまま", async () => {
    ...
    const patchA = (wrapper.emitted("event")![0]![0] as { value: FamDraftPatch }).value;
    const afterA = previewFamDraftPatch(openFamText(JSON.stringify(fam)), patchA);
    ...
    const patchB = (wrapper.emitted("event")![1]![0] as { value: FamDraftPatch }).value;
    const afterB = previewFamDraftPatch(openFamText(JSON.stringify(valueA)), patchB, { validate: validateFamJson });
