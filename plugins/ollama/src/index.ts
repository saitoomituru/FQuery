...
function buildPrompt(request: CapabilityInvocation): string {
  ...
  return JSON.stringify({ proton_profile: "proton://fquery/fam-json-core@0.1.0-draft", capability: request.capability, source: request.input, source_language_hint: sourceLanguageHint, translation_target: translationTarget, ref_profiles: request.profileBindings ?? [], instruction: "Return one fam.json/0.1.0-draft record. Apply every ref_profiles entry whose roles contains generation-constraint using exactly its profileRef and revisionRef; do not invent an undeclared hierarchy, World, Perspective, authority, causality, or boundary rule. The input language is the canonical origin language: never replace its title, index_subjects, Q.unknowns source_expression, ψ, ∇φ source_expression, or λ manifestation with a translation. Set λ.purpose exactly to the machine token source-decomposition. Set title_language and every source_language/manifestation_language consistently. λ.output_units must cover the complete source in original order without omission or duplication. Each output unit must preserve an exact source substring in ψ.source_text, every ∇φ[*].source_expression, and λ.manifestation, and must include Observer/Registry/fact-scope/unknown Q fields. Every Q.unknowns item is {source_expression:<exact input-language substring>,source_language:<input language>,concept_id:<machine identifier>}; do not put an English label in source_expression. Put other-language natural-language text only in λ.sub_splitters as a nested ψ/∇φ/λ/Q translation-witness FAM. Its ψ retains the canonical source text plus source_language and target_language; its λ.manifestation is the translation and manifestation_language equals target_language; its Q contains copy_role=translation-witness, source_node_ref, unknowns=[], unknown_is_absence=false, and translation_error={status:not-evaluated,metric_refs:[],measurements:[]}. Keep provenance structured and do not add other-language prose there. Do not return blocks[], RPC/MCP envelopes, FAMLog, or transport events." });
}
